...
- [x] `[cli|mcp|desktop]` Dangerous actions (kill, delete, restart) require explicit `--allow-*` flag (2026-02-27) — training runs are time-boxed by default (10 min), resource governance prevents runaway processes
- [x] `[cli|mcp|desktop]` File operations constrained to known directories (2026-02-27) — Zod schema rejects path traversal (`..`), writes to project directory only
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[mcp]` SKIP: not an MCP server

## B. Error Handling

- [x] `[all]` Errors follow the Structured Error Shape: `code`, `message`, `hint`, `cause?`, `retryable?` (2026-02-27) — Zod validation errors with structured messages
- [x] `[cli]` Exit codes: 0 ok · 1 user error · 2 runtime error · 3 partial success (2026-02-27)
- [x] `[cli]` No raw stack traces without `--debug` (2026-02-27) — ora spinner shows clean error messages
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[desktop]` SKIP: not a desktop application (VS Code extension is separate)
- [ ] `[vscode]` SKIP: VS Code extension is a companion, not the primary product
...
- [x] `[cli|mcp|desktop]` Logging levels defined: silent / normal / verbose / debug — secrets redacted at all levels (2026-02-27) — ora spinners for user-facing output, debug logging via Node.js
- [ ] `[mcp]` SKIP: not an MCP server
...
- [ ] `[vsix]` SKIP: VS Code extension is a companion product
